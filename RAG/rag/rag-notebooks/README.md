...
A 14-notebook, hands-on course that builds a Retrieval-Augmented Generation (RAG) system from the ground up — one concept per notebook, every example run against one real document instead of toy data.
...
- **Notebook 14** adds observability and evaluation on top of that agent: MLflow prompt versioning, autologged traces, and LLM-as-judge evaluation.
...
| # | Notebook | What it covers |
|---|---|---|
| 14 | [14_mlflow_tracing_and_evaluation.ipynb](14_mlflow_tracing_and_evaluation.ipynb) | Wires notebook 13's LangGraph agent into **MLflow**, unchanged in behavior: the system prompt becomes a versioned entry in MLflow's **Prompt Registry** (`register_prompt`/`load_prompt`, loaded by `champion` alias), `mlflow.langchain.autolog()` turns every turn into a full **trace** (LLM + tool calls, nested), and `mlflow.genai.evaluate()` scores the agent over a labeled question set with LLM-as-judge scorers — MLflow's built-in `Correctness`/`Guidelines`/`RelevanceToQuery` plus one fully custom judge (`custom_prompt_judge`). Requires a local MLflow tracking server running at `http://localhost:5000`. |
...
- **A `.env` file in this folder with `GOOGLE_API_KEY`** — required only for notebooks 11–14 (Gemini generation). Get a free key from [Google AI Studio](https://aistudio.google.com/apikey); the `gemini-3-flash-preview` model used in these notebooks is free-tier-eligible. `.env` is git-ignored, so your key won't be committed.
- **No key needed** for notebooks 1–10, or for the web-search tool in notebooks 12–14 (`ddgs` queries DuckDuckGo directly).
- **A local MLflow tracking server for notebook 14** — start one with `mlflow server --host 127.0.0.1 --port 5000` before running that notebook; it logs prompts, traces, and evaluation runs there.
- Notebooks 1–13 are written to run **either in Google Colab or locally** — each has its own `%pip install` setup cell and a Colab file-upload fallback (`maybe_colab_upload()`), so no notebook depends on a prior one's kernel state. Notebook 14 depends on a `localhost` MLflow server, so it's written to run locally only.

## Other notes

- **Run order matters conceptually, not mechanically.** Each notebook rebuilds everything it needs from scratch (loading, chunking, embedding, indexing), so any notebook can be opened and run standalone — but the material builds progressively, and later notebooks assume you've seen the ideas from earlier ones (e.g. notebook 8 assumes you understand the two retrievers from notebook 7). Going 1 → 14 in order is recommended for first-time viewers.
...
- **Chapter list of `rag.pdf`:** 01 Introduction to RAG · 02 Evolution of Retrieval · 03 Data Ingestion · 04 Embeddings · 05 Vector Databases & Indexing · 06 Retrieval Techniques · 07 Retrieval Evaluation · 08 Re-ranking · 09 Augmentation · 10 Generation · 11 Generation Evaluation · 12 RAG vs Fine-tuning · 13 RAG with Tools · 14 Agentic RAG · 15 Production Best Practices. Notebooks 1–10 and 12–13 map closely onto chapters 1–8 and 13–14; notebook 14's LLM-as-judge evaluation revisits chapter 11 (Generation Evaluation), this time for generated answers rather than retrieval; chapter 15 (production practices) isn't a dedicated notebook.
